...
const AdminHome = () => {
	const fetchTweets = trpc.tweet.fetchTweets.useQuery(undefined, {
		refetchOnMount: true,
	});
	const seedTweet = trpc.tweet.adminSeedTweet.useMutation({
		onSettled: () => {
			fetchTweets.refetch();
		},
	});
	...
	return (
		<Box p={2}>
			...
			<Button
				onClick={() => seedTweet.mutate()}
				isLoading={seedTweet.isLoading}
				marginY={2}
			>
				Seed Tweet
			</Button>
			...
			{fetchTweets.data &&
				fetchTweets.data.map((tweet, index) => {
					return (
						<Box
							mt={3}
