...
export const tweetRouter = t.router({
  ...
  createTweet: t.procedure.input(z.object({
    title:z.string().min(1,{message:"Title tweet cannot be empty"}),
    content:z.string().min(1,{message:"Content cannot be empty"}), 
  })).mutation(async (opts)=>{
    const user = await GetUserSessionUseCase.execute()
    const {title,content} = opts.input
    const newTweet = await CreateTweetUseCase.execute(title,content,user.user_id)
    return {
      data:newTweet,
      message:"Tweet created successfully"
    }
  }),
  deleteTweet: t.procedure.input(z.object({
    tweet_id:z.number(), 
  })).mutation(async (opts)=>{
    const user = await GetUserSessionUseCase.execute()
